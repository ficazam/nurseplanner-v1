...
const Patients = async () => {
  ...
  const surgeons: any[] = await prisma.user.findMany({
    where: { userType: "Cirujano" },
  });
  const nurses: any[] = await prisma.user.findMany({
          ...
          <Card key={patient.id}>
            <Link href={`/patients/${patient.id}`}>
              <div className="flex flex-col items-start justify-center w-full gap-y-2">
                ...
                <p>
                  Enfermero Asignado:{" "}
                  {nurses.find((nurse) => nurse.id === patient.nurseId)?.name}
                </p>
                <p>Dirección: {patient.address}</p>
                <p>Número de teléfono: {patient.phoneNumber}</p>
